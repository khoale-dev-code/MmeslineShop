...
def _sync_collection_members(collection_id: str) -> None:
    db = _db_admin()
...
def add_collection():
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            return render_template("admin/collection_form.html", cat=None, collection_products_catalog=product_picker)
        ...
        _sync_collection_members(str(created.get("id")))
...
def edit_collection(cid):
    ...
    if request.method == "POST":
        ...
        if not name:
            flash("Tên bộ sưu tập không được để trống.", "danger")
            return render_template("admin/collection_form.html", cat=cat, collection_products_catalog=product_picker)
