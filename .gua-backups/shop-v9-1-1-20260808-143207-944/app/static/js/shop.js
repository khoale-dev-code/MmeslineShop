(function () {
  ...
  function openQuickView(card, trigger) {
    ...
    if (category) category.textContent = card.dataset.category || "GUAMAISON";
    if (price) price.textContent = card.dataset.priceText || "";
  ...
  async function addToCart(card, variantId, button) {
    ...
    try {
      ...
      notify(data.message || "Đã thêm vào giỏ hàng", "success");
      window.dispatchEvent(new CustomEvent("cart:updated", { detail: data }));
      if (state.activeLayer === one(SELECTOR.quick)) closeLayer();
    } catch (error) {
      notify(error.name === "AbortError" ? "Kết nối quá chậm, vui lòng thử lại" : error.message || "Không thể thêm vào giỏ hàng", "error");
    } finally {
      if (button) {
        delete button.dataset.busy;
  ...
  window.GUAMaisonShopV9 = Object.freeze({ init: scheduleInit });
