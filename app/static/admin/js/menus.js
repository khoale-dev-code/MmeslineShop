(function () {
  ...
  const catalogs = {
    product: (initial.products || []).map((item) => ({ id: String(item.id), label: item.name, url: `/product/${item.slug}`, active: item.is_active !== false })),
    category: (initial.categories || []).map((item) => ({ id: String(item.id), label: item.name, url: `/shop?category=${encodeURIComponent(item.slug)}`, active: item.is_active !== false })),
    collection: (initial.collections || []).map((item) => ({ id: String(item.id), label: item.name, url: `/collections/${encodeURIComponent(item.slug)}`, active: item.is_active !== false })),
    page: [
  ...
  function renderTree() {
    ...
    $("#menuTree").innerHTML = rows.length ? rows.map(({ item, depth }) => `
      <div class="hm-item ${item.enabled === false ? "opacity-50" : ""} ${item.target_missing ? "is-broken" : ""} ${item.target_inactive ? "is-inactive" : ""}" data-item-id="${escapeHtml(item.id)}" data-depth="${depth}">
        <span class="hm-drag" title="Sắp xếp"><i class="fa-solid fa-grip-vertical"></i></span>
        <span class="min-w-0"><strong class="hm-item-title block truncate">${escapeHtml(item.label)}${item.target_missing ? ' · <span class="text-rose-600">Liên kết hỏng</span>' : item.target_inactive ? ' · <span class="text-amber-600">Đích đang ẩn</span>' : ""}</strong><small class="hm-item-meta block truncate">Cấp ${depth} · ${escapeHtml(item.link_type || "url")} · ${escapeHtml(item.url || "#")}</small></span>
        <span class="hm-actions">
          ...
          <button type="button" class="hm-icon" data-item-action="duplicate" title="Nhân bản"><i class="fa-regular fa-copy"></i></button>
  ...
  function duplicateMenu() {
    const source = currentMenu();
    if (!source) return;
    if (menus.length >= 30) return notify("Tối đa 30 menu.", "error");
    const title = `${source.title} - Bản sao`;
    const clone = structuredClone(source);
    clone.id = makeId("menu");
    clone.title = title.slice(0, 100);
    clone.handle = uniqueHandle(title);
    flatten(clone.items || []).forEach(({ item }) => { item.id = makeId("link"); });
    menus.push(clone);
    selectedHandle = clone.handle;
    renderPlacements(); loadSelectedMenu(); markDirty();
  }
  async function deleteMenu() {
    ...
    const ok = await askDelete({
      title: "Xóa menu",
      message: `Bạn có muốn xóa menu “${menu.title}”?`,
      detail: "Toàn bộ liên kết và menu con bên trong cũng sẽ bị xóa sau khi bạn bấm Lưu cấu hình.",
  ...
  function refreshTarget(selectedId = "") {
    ...
    const options = targetOptions(type).map((item) => ({ ...item }));
    const targetField = $("#targetField");
    const urlField = $("#urlField");
    if (selectedId && !options.some((item) => item.id === String(selectedId))) {
      options.unshift({ id: String(selectedId), label: "Nội dung không còn tồn tại", url: "#", missing: true });
    }
    ...
    $("#itemTarget").innerHTML = options.map((item) => `<option value="${escapeHtml(item.id)}" data-url="${escapeHtml(item.url)}" data-label="${escapeHtml(item.label)}" data-missing="${item.missing ? "true" : "false"}">${escapeHtml(item.label)}${item.active === false ? " (đang ẩn)" : ""}</option>`).join("");
    if (selectedId && options.some((item) => item.id === String(selectedId))) $("#itemTarget").value = String(selectedId);
    $("#itemTargetSearch").value = "";
    $("#itemTargetHint").textContent = hasPicker ? `${options.length} nội dung · liên kết được tự sửa URL khi slug thay đổi` : "";
    syncTarget();
  }
  function filterTargets() {
    const keyword = $("#itemTargetSearch").value.trim().toLowerCase();
    let visible = 0;
    Array.from($("#itemTarget").options).forEach((option) => {
      const show = !keyword || (option.dataset.label || option.textContent).toLowerCase().includes(keyword);
      option.hidden = !show;
      if (show) visible += 1;
    });
    $("#itemTargetHint").textContent = `${visible} kết quả phù hợp`;
  }
  function syncTarget() {
    const type = $("#itemLinkType").value;
    if (type === "url") return;
  ...
  function saveItem() {
    ...
    if (["product", "category", "collection", "page", "home"].includes(type) && !target) return notify("Vui lòng chọn nội dung liên kết.", "error");
    const payload = {
  ...
  async function actOnItem(id, action) {
    const entry = findEntry(id);
    if (!entry) return;
    if (action === "edit") return editItem(id);
    if (action === "duplicate") {
      const clone = structuredClone(entry.item);
      flatten([clone]).forEach(({ item }) => { item.id = makeId("link"); });
      clone.label = `${clone.label} - Bản sao`.slice(0, 100);
      entry.array.splice(entry.index + 1, 0, clone);
    } else if (action === "remove") {
      const ok = await askDelete({
        title: "Xóa liên kết menu",
        message: `Bạn có muốn xóa liên kết “${entry.item.label}”?`,
        detail: "Toàn bộ liên kết con bên trong cũng sẽ bị xóa sau khi bạn bấm Lưu cấu hình.",
  ...
  async function saveAll() {
    ...
    try {
      ...
      if (result.navigation?.menus) {
        const keepHandle = selectedHandle;
        menus = structuredClone(result.navigation.menus);
        placements = { ...(result.navigation.placements || placements) };
        selectedHandle = menus.some((menu) => menu.handle === keepHandle) ? keepHandle : menus[0]?.handle || "";
        renderPlacements(); loadSelectedMenu();
      }
      dirty = false;
  ...
  root.addEventListener("click", (event) => {
    ...
    if (event.target.closest("#duplicateMenuBtn")) return duplicateMenu();
  ...
  root.addEventListener("input", (event) => {
    if (event.target.id === "itemTargetSearch") return filterTargets();
